...
def neighb_square(sol, scale, domain_width):
    ...
    # Need to copy, because Python pass by reference
    # and we don't want to change the original solution.
    new = copy.copy(sol)
    for py in range(len(sol)):
        for px in range(len(sol[py])):
            if sol[py][px] == 1:
                ...
                if py+y(d) < 0 :
                    d[1] = np.random.randint(-py,scale//2,1)
                if py+y(d) >= domain_width :
                    d[1] = np.random.randint(-scale//2,domain_width-py,1)
                if px+x(d) < 0 :
                    d[0] = np.random.randint(-px,scale//2,1)
                if px+x(d) >= domain_width :
                    d[0] = np.random.randint(-scale//2,domain_width-px,1)
                new[py+y(d)][px+x(d)] = 1
    return new
